...
for i_episode in range(1, num_episodes + 1):
    ...
    for i_game in range(num_game_each_episode):
        ...
        if (i_episode>100 and random.random()>0.93 ):
            epsss =1
        elif(i_episode>100):
            epsss = 0
        ...
        reward_p1 =  agent_p1.touch_ball_reward + goal_reward_p1 #+ agent_p1.close_ball_reward  +agent_p1.rew_d_to_our_post# +agent_p1.rew_d_to_our_post
        #reward_b1 =  agent_b1.touch_ball_reward + goal_reward_b1 + agent_b1.close_ball_reward
        if(agent_p1.touch_ball_reward>0):
            agent_p1.upd_after_goal((min(30,i_game)))
        if(agent_p2.touch_ball_reward>0):
            agent_p2.upd_after_goal((min(30,i_game))) 
        if(goal_reward_p1>1):
            agent_p1.we_goll()
            agent_p2.we_goll()
        if(goal_reward_p1<-1):
        ...
        reward_p2 =  agent_p2.touch_ball_reward + goal_reward_p1+goal_reward_p2# + agent_p2.close_ball_reward + agent_p2.rew_for_ball_dist #+agent_p2.rew_d_to_our_post
        #reward_b2 =  agent_b2.touch_ball_reward + goal_reward_b1 + agent_b2.close_ball_reward
        print(reward_p1)
        print(reward_p2)
        #input('rew')
    ...
    if average_scores[0] > striker_solved_score and average_scores[1] > defender_solved_score:
        ...
        nn_p1_filename = 'p1_mod'
        nn_p2_filename = 'p2_mod'
        torch.save(agent_p1.network.state_dict(), nn_p1_filename)
        torch.save(agent_p2.network.state_dict(), nn_p2_filename)

    if i_episode % 50 == 0:
        timestr = time.strftime("%Y%m%d-%H%M%S")
        nn_last_p1_filename = 'p1_act_last'
        nn_last_target_p1_filename = "p1_target_last"

        nn_last_p2_filename = "p2_act_lat"
        nn_last_target_p2_filename = "p2_target_last"
